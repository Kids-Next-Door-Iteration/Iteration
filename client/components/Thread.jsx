...
function Thread() {
      ...
      <div className='post'>
        <div className='post-header'>
          <UserInfo
            ...
            email={msg.email}></UserInfo>
          <p>{msg['date']}</p>
        </div>
        <p className='post-content'>{msg['thread']}</p>
        <div className='post-buttons'>
          {currentUserEmail === msg.email && !replyClicked && (
            <button className='post-reply-button btn btn-outline-primary'>
              Delete
            </button>
          )}
          {!replyClicked && (
            <button
              className='post-reply-button btn btn-primary'
              onClick={() => {
                reply(msg._id);
              }}>
              Reply
        ...
        {replyClicked && currentThreadId === msg._id && (
          <div className='reply-box'>
            ...
            <button className='btn btn-outline-primary' onClick={cancelReply}>
              Cancel
            </button>
            <button
              type='button'
  ...
  return (
    <div className='thread-page'>
      ...
      <div id='thread-container'>{postList}</div>
    </div>
  );
}
