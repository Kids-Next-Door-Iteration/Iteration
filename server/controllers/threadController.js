...
threadController.createPost =
	async (
		...
		const queryUser =
			'SELECT u._id FROM users u WHERE email = $1;';
		const paramsUser =
			[
				email,
			];
  ...
  RETURNING threads.thread, threads.date;';
		...
		try {
			...
			res.locals.eventName =
				eventId.rows[0].event_name;
			paramsThread.push(
				eventId
					.rows[0]
					._id
			...
			res.locals.threadData =
				threadData.rows[0];
			res.locals.threadId =
				threadData.rows[0]._id;
...
threadController.getUpcomingEvents =
	async (
		req,
		res,
		next
...
threadController.deleteThread =
	async (
		...
		const queryDeleteThread =
			'DELETE FROM threads WHERE event_id = $1;';
		const queryDeleteEvent =
			'DELETE FROM event WHERE _id =$1;';
