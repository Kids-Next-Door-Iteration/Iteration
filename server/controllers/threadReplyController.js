...
threadReplyController.postThreadReply =
	async (
		req,
		res,
		next
		...
		const queryEmail =
			'SELECT users._id FROM users WHERE email = $1;';
		const emailParams =
			[
				email,
			];
		...
		const queryEvent =
			'SELECT event._id FROM event WHERE event_name =$1;';
		const eventParams =
			[
				event_name,
			];
		...
		const queryRes =
			'INSERT INTO threads (thread, date, thread_id, user_id, event_id) VALUES ($1, $2, $3, $4, $5) RETURNING *;';
		const resParams =
			[
				thread,
				date_posted,
	...
	async (
		...
		const id =
			req
				.params
				.id;
		const deleteReplyParam =
			[
				id,
			];
		const queryDeleteRes =
